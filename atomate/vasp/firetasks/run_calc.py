...
class RunVaspCustodian(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        c = Custodian(handlers, jobs, validators=validators, max_errors=max_errors,
                      scratch_dir=scratch_dir, gzipped_output=gzip_output, terminate_func=terminate_func)
...
@explicit_serialize
class RunPhonopy(FiretaskBase):
    """
    Calculate dispersion with phonopy.

    Required params:
        vasp_cmd (str): the name of the full executable for running VASP.
        supercell: the size of supercell (TODO)
    """
    required_params = ["vasp_cmd"]
    try:
        from phonopy import Phonopy
    except ImportError:
        logger.warn("The required 'phonopy' package NOT installed.")
    ...
    from phonopy.interface import create_FORCE_SETS
    import phonopy.file_IO as file_IO
    import fnmatch

    def run_task(self, fw_spec):
        print("Firetask: run phonopy")
        supercell = [[5, 0, 0], [0, 5, 0], [0, 0, 5]] # Parameters (TODO!)
        unitcell, opt_info = read_crystal_structure(filename="POSCAR")
        phonon = Phonopy(unitcell,supercell)
        print("supercell generated")
        ...
        for file_name in os.listdir("."):
            if fnmatch.fnmatch(file_name,"POSCAR-*"):
                disp_index = file_name.split("-")[-1]
                folder_name = "disp-"+disp_index
                os.system("mkdir "+folder_name)
                os.system("cp "+file_name+" "+folder_name+"/POSCAR")
                os.system("cp INCAR KPOINTS POTCAR "+folder_name)
                os.chdir(folder_name)
                vasp_cmd = env_chk(self["vasp_cmd"], fw_spec)
                logger.info("Running VASP in folder: {}".format(folder_name))
                return_code = subprocess.call(vasp_cmd, shell=True)
                logger.info("VASP finished running with returncode: {}".format(return_code))
                os.chdir("..")
        
        folders_disp = fnmatch.filter(os.listdir("."),"disp-*")
        folders_disp.sort()
        for i in range(0, len(folders_disp)):
            folders_disp[i]=folders_disp[i]+"/vasprun.xml"
        create_FORCE_SETS("vasp",folders_disp)
